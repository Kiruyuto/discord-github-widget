...
This guide focuses on the repository-specific setup for this GitHub activity widget: configuration, GitHub credentials, widget layout mapping, and running the app.

## What You Will Set Up

- A Discord application, bot token, authorization URL, and install/profile flow using external resources
- This widget's layout mapping for Discord dynamic fields
- A GitHub OAuth app for Device Flow (optional)
- A GitHub API token for fetching public profile data

## Discord Setup Resources

Honestly, for the Discord-facing setup just use one of those resources, all of them written far better than I could ever have:

- [Chloe's Blog](https://chloecinders.com/blog/discord-widgets)
- [Aamia's Widget Creator Gist](https://gist.github.com/aamiaa/7cdd590e3949cd654758bc90bcb4710b)
- [NTTS YT Vid](https://www.youtube.com/watch?v=gYv7D83u7yQ)
- [Discord Previews Server](https://discord.gg/discord-603970300668805120)

After following those resources, you should have:

- `Discord__Token`: the bot token used by this app.
- `Discord__AuthorizeUrl`: the authorization URL shown in the `/setup` flow.
- Access to the widget layout editor so you can apply the mapping in [Widget Layout Mapping](#2-widget-layout-mapping).
...
## 1. Complete the Discord Setup Resources

Follow the resources in [Discord Setup Resources](#discord-setup-resources) before running this app.  
This guide expects those resources to produce the values used by this repository:
- `Discord__Token`
- `Discord__AuthorizeUrl`

Keep those values private and never share them with anyone.

## 2. Widget Layout Mapping

After the Discord setup resources get you to the widget layout editor, use this section to map the fields for this project.
...
- The application uses GitHub OAuth Device Flow, so the callback URL is not used by this project at runtime. GitHub still requires one when creating an OAuth app, so you can use the same URL you created while following the Discord setup resources.
